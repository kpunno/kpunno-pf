...
export default function Astroboy() {
  ...
  const [canvas_x, canvas_y] = canvasSize;
  const canvasRef = useRef(null);
  useEffect(() => {
    if (canvasRef.current) {
      setSimulator(SimulatorWidget(canvasRef.current));
    }
  }, [canvasSize]);

  async function handleAutoRun() {
    if (simulator) {
      await simulator.autoRun();
    }
  }
  ...
  function SimulatorWidget(canvas) {
    var ui = UI();
    var display = Display();
    var memory = Memory();
    var labels = Labels();
    ...
    async function autoRun() {
      ...
      var code = await ui.loadCodeFile();
  ...
  */
    ...
    function UI() {
      ...
      async function loadCodeFile() {
        let code;
        try {
          code = await fetch("/asm/astroboy.asm").then((res) => res.text());
          return code;
        } catch (err) {
          console.log(err);
        }
    ...
    function Simulator() {
      ...
      var instructions = {
        ...
        ierr: function () {
          console.log("Address $" + addr2hex(regPC) + " - unknown opcode");
          codeRunning = false;
        },
      };
      ...
      function stackPush(value) {
        ...
        if (regSP < 0) {
          regSP &= 0xff;
          console.log("6502 Stack filled! Wrapping...");
        }
      }
      ...
      function stackPop() {
        ...
        if (regSP >= 0x100) {
          regSP &= 0xff;
          console.log("6502 Stack emptied! Wrapping...");
        }
      ...
      function gotoAddr() {
        ...
        if (addr === 0) {
          console.log("Unable to find/parse given address/label");
        } else {
          regPC = addr;
        }
        updateDebugInfo();
      ...
      function stop() {
        codeRunning = false;
        console.log("\nStopped\n");
        document.removeEventListener("keydown", memory.storeKeydown);
      }
      ...
      function assembleCode(code) {
        ...
        if (codeAssembledOK) {
          ...
          if (!wasOutOfRangeBranch) {
            console.log("**Syntax error line " + (i + 1) + ": " + str + "**");
          } else {
            console.log(
        ...
        console.log("Code assembled successfully, " + codeLen + " bytes.");
        return true;
      }
